...
## 2026-09-05 — lista stakala, tip porudžbine za kontaktna sočiva, procureli znak mrtvog tastera

- **Padajuća lista predloga za stakla — sopstvena umesto nativnog `<datalist>`**: klik u prazno polje "naziv stakla" nije otvarao spisak ranije unetih stakala (predlozi su se pojavljivali tek posle par otkucanih slova). Prvi pokušaj (forsiranje kroz `showPicker()` na `onfocus` i `onclick`) je pogoršao stvar — lista se otvarala pa odmah zatvarala, "treperila", jer su je otvarala tri izvora odjednom (nativni klik + dva naša poziva). Zamenjeno sopstvenom listom u `js/utils.js`: atribut `list=` se skida sa polja čim se ono pojavi u DOM-u (`MutationObserver` hvata i redove stakala koje `renderLensRows()` iscrtava kroz `innerHTML`), pa nativna lista nikad ne iskoči; vrednosti se čitaju iz iste `<datalist>` liste koju puni `loadLensAutocompleteData()`, pa `orders.js` i `crm.html` nisu menjani. Ponašanje: klik/fokus → ceo spisak, kucanje → filtriranje, strelice + Enter → izbor, Esc/klik van → zatvaranje. Izbor stavke šalje `input` event, pa `onLensNameInput()` i automatska cena iz kataloga rade kao pri kucanju; Enter bez označene stavke se prosleđuje postojećoj logici (`handleLensEnterJump`, `initEnterNavigation`).
- **Porudžbina posle recepta za kontaktna sočiva otvara pogrešan tab**: `openAddOrderModal()` uvek startuje na "Naočare", pa se i posle recepta sa namenom "kontaktna sočiva" forma otvarala na naočarima. `savePrescriptionForm()` u `js/prescriptions.js` sada, ako lanac recepata sadrži "kontaktna sočiva", odmah poziva `setOrderType('contact_lenses')`.
- **Procureli znak mrtvog tastera u polju "Ime" (treći pokušaj — sada rešeno)**: znak koji se pojavljivao nije bio "ё" nego "‚" (U+201A). Taster levo od "1" je na srpskoj latiničnoj raspored **mrtav taster** (dead key): OS ga ne ispisuje odmah, nego ga ubaci **zajedno sa prvim stvarnim slovom** koje se otkuca u novootvorenom polju — otud "‚Vuk" umesto "Vuk". Zato ni `preventDefault()` na keydown, ni `e.repeat`-guard (2026-08-18, deo 1), ni composition-guard (2026-08-18, deo 2) nisu mogli da ga uhvate: ta zaštita je brisala samo unos pristigao PRE prvog pravog `keydown`-a, a ovde znak stiže tačno u istom potezu sa slovom, posle pravog `keydown`-a. Ispravljeno u `js/nav.js`: `guardAgainstLeakedShortcutChar()` sada, u prozoru od 5 sekundi posle prečice, sa POČETKA vrednosti svih tekstualnih polja forme skida eventualni procureli znak (`` ` `` ~ ё ‚ ‛ „ ‘ ’ ¸ ¨ ° ' "), a ostatak unosa ostaje netaknut; provera duplikata se ponavlja sa očišćenom vrednošću. Van tog prozora i van prečice ne dira se ništa.

Sve provereno Playwright testom protiv stvarnih fajlova repoa (stubovani `config.js`/`auth.js`, 13 provera): procureli "‚" uz prvo slovo, "ё" sam pre kucanja, "`" uz slovo, normalan unos posle prečice, unos van prozora zaštite, otvaranje liste stakala na klik i da lista ne nestaje, izbor iz liste sa automatskom cenom, i porudžbina koja se otvara na tabu "Kontaktna sočiva".
...
- ⚠️ **Za proveru**: `patients.visit_date` ("datum posete", koristi se kao datum prvog obraćanja) nije uvek pouzdan — nađeno par slučajeva gde ne odgovara stvarnoj istoriji recepata/porudžbina pacijenta (npr. jedan pacijent: recepti iz 2024/2025, `visit_date` = 2026-08-01; drugi pacijent: `visit_date` je 9 dana POSLE recepta). Nije automatski ispravljeno — pravilo koje bi trebalo primeniti nije bilo jasno definisano, ostaje na Aninu odluku.
...
- **Nađeno u bazi**: 12 takvih porudžbina, sve vezane za pacijente koje je Anna već ranije obrisala kao test/probne unose (dva test-naloga "Anna Novoselova", jedan test-pacijent "Ervin Ginter", dva unosa "222"/"222", jedan "test"/"test", jedan dupliran test unos druge test-pacijentkinje). Ukupno 143.766 din. lažnog prometa je ušlo u analitiku kroz ove porudžbine (raspon jun–avgust 2026). Sve 12 su soft-obrisane (`deleted_at`) direktno u Supabase-u — ostaju u bazi radi istorije, ali se više ne broje.
